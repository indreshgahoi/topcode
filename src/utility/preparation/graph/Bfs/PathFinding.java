...
public class PathFinding {
	...
	public boolean valid(state st) {
		return ((st.p1x < 0 || st.p1y < 0 || st.p2x < 0 || st.p2y < 0 || st.p1x >= board[0].length()
				|| st.p1y >= board[0].length() || st.p2x >= board[0].length() || st.p2y >= board[0].length()
				|| board[st.p1x].charAt(st.p1y) == 'X' || board[st.p2x].charAt(st.p2y) == 'X')
				|| ((st.p1x == st.p2x) && (st.p1y == st.p2y)));
	}
				...
				for (int p1dy = -1; p1dy <= 1; p1dy++) {
					for (int p2dx = -1; p2dx <= 1; p2dx++) {
						for (int p2dy = -1; p2dy <= 1; p2dy++) {
							...
							if ((ft.p1x == (ft.p2x + p2dx)) && (ft.p1y == (ft.p2y + p2dy))
									&& ((ft.p2x == (ft.p1x + p1dx)) && (ft.p2y == (ft.p1y + p1dy))))
								continue;
							que.add(new state(ft.p1x + p1dx, ft.p1y + p1dy, ft.p2x + p2dx, ft.p2y + p2dy,
									ft.steps + 1));
						}
					}
				}
